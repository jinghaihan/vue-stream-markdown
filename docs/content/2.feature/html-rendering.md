...
HTML support is built into `vue-stream-markdown`; there is no HTML package or plugin to install. Comark's default HTML parser converts supported native HTML and custom HTML-like tags into its normal document tuples. Vue Stream Markdown renders native tags directly and lets you map custom tags through `components`; no separate HTML representation or renderer is involved.
