...
Emitted after the smoothing queue is drained, the final Markdown parse completes, and Vue has flushed the rendered nodes. In streaming UIs, this is useful for actions that must wait for the complete document, such as scrolling to the final rendered content.
