...
export interface CompletionParagraphAnalysis {
...
export interface CompletionAnalysis {
  ...
  isFullyCodeBlock: boolean
  lines: string[]
  getLastParagraph: (skipTrailingEmpty?: boolean) => CompletionParagraphAnalysis
}

const analysisCache = new WeakMap<CompletionContext, CompletionAnalysis>()
...
function analyzeCompletionContent(content: string): CompletionAnalysis {
  return new CachedCompletionAnalysis(content)
}
...
export function getCompletionAnalysis(
  content: string,
  context?: CompletionContext,
): CompletionAnalysis {
  if (!context)
    return analyzeCompletionContent(content)

  const cached = analysisCache.get(context)
  if (cached?.content === content)
    return cached

  const analysis = analyzeCompletionContent(content)
